...
<html lang="en" class="h-100">
  ...
  <div class="wrapper">
  	<nav class="navbar navbar-light bg-light" style="">
	  	<a class="navbar-brand" href="<c:url value="/" />"><c:if test="${not empty exhibition.title}">${exhibition.title}</c:if><c:if test="${empty exhibition.title}">Virtual Spaces</c:if></a>
		...
		<ul class="navbar-nav ml-auto" style="flex-direction: row">
		    ...
		    <form action="<c:url value="/logout" />" method="POST">
  				...
  				<button class="btn navbar-link" type="submit" title="Logout"><i class="fas fa-sign-out-alt"></i></button>
         	</form>
         	</li>
		    </sec:authorize>
        </ul>
    ...
    <nav id="sidebar" class="mCustomScrollbar active float-left position-absolute">
        ...
        <c:forEach items="${allSpaces}" var="space">
		  <a href="<c:url value="/exhibit/space/${space.id}" />" class="list-group-item list-group-item-action"><i class="fas fa-grip-horizontal"></i> ${space.name}</a>
		</c:forEach>
		</div>
    </nav>
	...
	<footer class="footer mt-auto">
	  <div class="d-flex justify-content-center">
	    <span class="text-muted">This web application was built with Virtual Spaces 2.0 (https://github.com/diging/virtual-spaces-2.0).</span>
	  </div>
	</footer>
